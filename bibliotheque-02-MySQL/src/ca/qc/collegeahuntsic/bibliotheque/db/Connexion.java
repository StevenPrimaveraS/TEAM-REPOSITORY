 ...
 *
 * Ce programme ouvrir une connexion avec une BD via JDBC. La m�thode
 * serveursSupportes() indique les serveurs support�s.
 *
 * Pr�-condition le driver JDBC appropri� doit �tre accessible.
 *
 * Post-condition la connexion est ouverte en mode autocommit false et
 * s�rialisable, (s'il est support� par le serveur).
 * </pre>
 */
public class Connexion {
	...
	public Connexion(String serveur, String bd, String user, String pass) throws SQLException {
		...
		try {
			if (serveur.equals("local")) {
				...
				this.conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + bd, user, pass);
			}
			if (serveur.equals("distant")) {
				d = (Driver) Class.forName("oracle.jdbc.driver.OracleDriver").newInstance();
				DriverManager.registerDriver(d);
				this.conn = DriverManager.getConnection("jdbc:oracle:thin:@collegeahuntsic.info:1521:" + bd, user,
						pass);
			}
			/*
			...
			if (dbmd.supportsTransactionIsolationLevel(Connection.TRANSACTION_SERIALIZABLE)) {
				...
				System.out.println("Ouverture de la connexion en mode s�rialisable :\n" + "Estampille "
						+ System.currentTimeMillis() + " " + this.conn);
			} else {
	...
	public void fermer() throws SQLException {
		this.conn.rollback();
		this.conn.close();
		System.out.println("Connexion ferm�e" + " " + this.conn);
	}
