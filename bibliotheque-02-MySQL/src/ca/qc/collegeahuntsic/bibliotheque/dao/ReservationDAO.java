...
public class ReservationDAO extends DAO {
	...
	private static final String UPDATE_REQUEST = "UPDATE reservation"
			+ "SET idLivre = ?, idMembre = ?, dateReservation = ?" + "WHERE idReservation = ?";
	...
	public boolean existe(int idReservation) throws DAOException {
		try (PreparedStatement statementExiste = getConnection().prepareStatement(ReservationDAO.READ_REQUEST)) {
			statementExiste.setInt(1, idReservation);
	...
	public ReservationDTO getReservationLivre(int idLivre) throws DAOException {
		try (PreparedStatement statementExisteLivre = getConnection()
				.prepareStatement(ReservationDAO.READ_REQUEST_LIVRE)) {
			statementExisteLivre.setInt(1, idLivre);
	...
	public ReservationDTO getReservationMembre(int idMembre) throws DAOException {
		try (PreparedStatement statementExisteMembre = getConnection()
				.prepareStatement(ReservationDAO.READ_REQUEST_MEMBRE)) {
			statementExisteMembre.setInt(1, idMembre);
	 ...
	 *             si une erreur survient
	 */
	public void reserver(int idReservation, int idLivre, int idMembre, String dateReservation) throws DAOException {
		try (PreparedStatement statementInsert = getConnection().prepareStatement(ReservationDAO.ADD_REQUEST)) {
			statementInsert.setInt(1, idReservation);
			statementInsert.setInt(2, idLivre);
			statementInsert.setInt(3, idMembre);
			statementInsert.setString(4, dateReservation);
	...
	public int annulerRes(int idReservation) throws DAOException {
		try (PreparedStatement statementDelete = getConnection().prepareStatement(ReservationDAO.DELETE_REQUEST)) {
			statementDelete.setInt(1, idReservation);
	...
	public void add(ReservationDTO reservationDTO) throws DAOException {
		try (PreparedStatement statementInsert = getConnection().prepareStatement(ReservationDAO.ADD_REQUEST_2)) {
			/* Ajout du reservation. */
			statementInsert.setInt(1, reservationDTO.getIdReservation());
			statementInsert.setInt(2, reservationDTO.getIdLivre());
			statementInsert.setInt(3, reservationDTO.getIdMembre());
			statementInsert.setDate(4, reservationDTO.getDateReservation());
			statementInsert.executeUpdate();
		} catch (SQLException sqlException) {
			throw new DAOException(sqlException);
		}
	...
	public ReservationDTO read(int idReservation) throws DAOException {
		...
		try (PreparedStatement statementExisteMembre = getConnection()
				.prepareStatement(ReservationDAO.READ_REQUEST_MEMBRE)) {
			statementExisteMembre.setInt(1, idReservation);
			try (ResultSet resultset = statementExisteMembre.executeQuery()) {
				if (resultset.next()) {
	...
	public void update(ReservationDTO reservationDTO) throws DAOException {
		try (PreparedStatement statementUpdate = getConnection().prepareStatement(ReservationDAO.UPDATE_REQUEST)) {
			/* Ajout du reservation. */
			statementUpdate.setInt(1, reservationDTO.getIdLivre());
			statementUpdate.setInt(2, reservationDTO.getIdMembre());
			statementUpdate.setDate(3, reservationDTO.getDateReservation());
			statementUpdate.setInt(4, reservationDTO.getIdReservation());
	...
	public void delete(ReservationDTO reservationDTO) throws DAOException {
		try (PreparedStatement statementDelete = getConnection().prepareStatement(ReservationDAO.DELETE_REQUEST)) {
			statementDelete.setInt(1, reservationDTO.getIdReservation());
